...
def deploy_predict(feature_pd, gpu_id=-1):
    ...
    required_paths = [
        deploy_paths["xgb_path"],
        deploy_paths["arcface_path"],
        deploy_paths["scaler_path"],
        deploy_paths["params_path"]
    ]
    missing_files = [path for path in required_paths if not os.path.isfile(path)]
    if missing_files:
        raise FileNotFoundError("Missing deployment model file(s): " + ", ".join(missing_files))
    ...
    if X_test_original.ndim != 2 or X_test_original.shape[1] != 2125:
        raise ValueError(f"Expected 2125 handcrafted features, got shape {X_test_original.shape}")
