...
# Import custom feature extraction and model prediction functions
...
try:
    ...
    st.error(f"Unable to load the feature extraction or model prediction module: {e}")
    st.stop()
...
def validate_peptide(sequence):
    ...
    if len(sequence) > 50:
        return False, "Sequence too long (max 50 amino acids)"
...
def process_sequences_background(seq_pd, progress_callback=None):
    ...
    for i, (_, row) in enumerate(processed_seq_pd.iterrows()):
        ...
        is_valid, validation_message = validate_peptide(sequence)
        if not is_valid:
            error_msg = f"Sequence {seq_id} validation failed: {validation_message}"
    ...
    for i, feature_item in enumerate(all_features_list):
        ...
        try:
            ...
            if len(prediction) > 0:
                ...
                raw_label = pred_row.get('label', 0)
                label = "Antiviral" if int(raw_label) == 1 else "Non-antiviral"
...
with st.expander("ℹ️ Model Information"):
    ...
    This tool uses the deployment model included under `ml_model/` for antiviral peptide prediction.

    **Workflow:**
    1. **FASTA Parsing**: Parse input sequences to a DataFrame
    2. **Feature Extraction**: Generate the handcrafted sequence descriptors
    3. **Model Prediction**: Generate ArcFace embeddings and predict with XGBoost
    4. **Result Display**: Show the predicted class and AVP probability

    **Input requirements:**
    - Standard amino-acid characters only: `ACDEFGHIKLMNPQRSTVWY`
    - Sequence length: 3-50 amino acids
    - Maximum 200 sequences per submission

    Model artifacts corresponding to the manuscript experiments are provided separately under `reproducibility/`.
    The manuscript and web-server model artifacts should not be used interchangeably.
    """)
